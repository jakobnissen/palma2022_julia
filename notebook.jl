...
The two languages have way too many differences for the notebook to show examples of them all, so only the most important are shown.
...
* Strength: Julia's type system
* Strength: Easy multiprocessing
"""
...
The compiled functions can be freely and dynamically overwritten, shadowed or extended, in which case the compiler will detect the changes, and re-compile any affected code before it is run the next time.

This way of working means that Julia is laggy and unresponsive when starting up. It needs to first compile all the Julia code used by your code editor, by the Julia REPL itself, or in the case for this notebook, the code used by the Pluto notebook system and all its dependencies. This causes latency of many seconds.
"""
...
The standard Julia code formatter `JuliaFormatter.jl` is backed by a much larger code base than `FASTX` and `BioSequences`, and is infamous for its long compile times.
...
You see the same delay again. BUT, now try to rerun the code and time it from the REPL:

```julia-repl
julia> @time (using JuliaFormatter; format("latency_test_1.jl"))
```

Do this a few times. It should be much faster than the first time.

Keeping the REPL open, try modifying the script, e.g. by formatting another Julia file. Then reload the file from the REPL with `include("latency_test_2.jl")`, and re-time running the function. What you will see is that only a small bit of the code needs to be recompiled, since most of the underlying plotting code is already compiled. As a result, there should be little latency.
"""
...
The two files `kmers.jl` and `kmers.py` do the same thing in Julia and Python, respectively. Read one (or both) of the files and figure out approximately what they do.

Time Julia the following ways:
* From command line (including startup latency): `time julia kmers.jl`
...
julia> @benchmark Kmers.count_matrix(Kmers.data, Kmers.matrix)
```
...
Alternatively, you can use the similar macros `@edit` or `@less`, e.g. `@less Set(1)` to use an editor or pager, respectively, to open up the source file, then you can look around the file to find what you're looking for.
...
* Open a new Julia REPL. Hit the square bracket close (`]`) key to enter _package management mode_, and type "`(palma2022_julia) pkg> generate MyProject`", then hit enter.
...
* Activate the new project by entering _package mode_ (backspace to go back to Julia mode, then hit `]`), then type "`(palma2022_julia) pkg> activate .`" - notice the ending dot for "the project in this directory". You should see the prompt change to `(MyProject) pkg>`.
...
If there is time to do this, in package mode, type `help` and look at the options. Try playing around with the package manager in your new environment.
"""
...
Python used to not really have a _type system_, i.e. a systematic and rigorous way of talking about the relationship between different types. However, after 10+ years of criticism, [a type system was introduced](https://docs.python.org/3/library/typing.html) in 2015 (Python 3.5), which is still being expanded and rolled out slowly in current Python releases.

The Python type system is awesome and a great improvement of the language. However, since the Python interpreter _itself_ has no concept of a type system, the system is artificially bolted on as a separate stdlib package, not integrated into the interpreter. For example, you can easily write code that violates the type system, and the interpreter wouldn't have a clue.

In contrast, Julia's compiler uses its type system heavily, so it is an integrated and natural part of the language.

Type systems (both Python's and Julia's) are inherently complex and heavy with technical terms and jargon. Instead of going into a deep dive which would be a week long course, let me show some concrete examples of the Julia type system in use:
"""
...
begin # 'begin' blocks are necessary in Pluto notebooks, not in Julia in general
	...
	default(::T) where {T <: AbstractString} = T("")
end
...
* Now launch JET and run an analysis of the top function call:
```julia-repl
julia> using JET
...
julia> @report_call mode=:sound Kmers.count_matrix(Kmers.data, Kmers.matrix)
```
...
function compute_vector(v)
	s = 0.0
	for i in v
		s += i / (isqrt(abs(i)) + round(Int, log(abs(i))))
	end
	s
end;
...
# ╔═╡ 25fa5cec-062f-4973-8888-928679de8a88
# Put your multithreaded function here
...
There are many more differences between Python and Julia than covered here. A few more of these deserves a mention though.

#### Other weaknesses of Julia
* Since Julia requires a JIT compiler to run, it also consumes around 150 MB memory just to run any script. This, combined with its latency makes it completely unsuitable for _deployment_ in e.g. a standalone app, or for a small background task. This limitation is being worked on, but it may take multiple years before Julia can create quick scrips without acceptable latency.

* Compared to Python, Julia is far more _immature_. This means major projects have fewer developers and have existed for a shorter time. As a consequence, the documentation for major projects is often of worse quality, and there are more bugs in the Julia ecosystem because there are fewer people to discover and fix them. There are also less likely to exist a Julia package for your specific needs compared to Python
